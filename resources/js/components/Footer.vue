<template>
    <footer class="px-5 md:px-20">
        <div class="flex flex-col items-center justify-center rounded-4xl md:flex-row">
            <picture class="md:w-1/4">
                <img src="/icons/HospitalDrRaulHernadez_vertical.svg" width="366" height="366" class="w-full h-full"
                    fetchpriority="high" decoding="async" loading="eager" alt="Logo pie pagina">
            </picture>
            <nav aria-label="Footer Navigation" class="flex flex-col justify-around w-3/4 gap-8 py-4 md:flex-row">
                <div>
                    <h3 class="mb-3 text-xl font-semibold">Servicios</h3>
                    <ul class="space-y-2">
                        <li><a href="#" class="hover:underline">Paquetes</a></li>
                        <li><a href="#" class="hover:underline">Especialidades </a></li>
                        <li><a href="#" class="hover:underline">Paquetes quirurgicos</a></li>
                    </ul>
                </div>
                <div>
                    <h3 class="mb-3 text-xl font-semibold">Nosotros</h3>
                    <ul class="space-y-2">
                        <li><a href="#" class="hover:underline">Misión</a></li>
                        <li><a href="#" class="hover:underline">Visión</a></li>
                        <li><a href="#" class="hover:underline">Valores</a></li>
                    </ul>
                </div>
                <div>
                    <h3 class="mb-3 text-xl font-semibold">Contáctanos</h3>
                    <ul class="space-y-2">
                        <li>
                            <span>
                                5 de Febrero 724, Col. Zona Centro, León, Gto. C.P. 37000
                            </span>
                        </li>
                        <li>
                            <span>477 714-19-73</span>
                        </li>
                    </ul>
                </div>
                <div>
                    <h3 class="mb-3 text-xl font-semibold">Siguenos</h3>
                    <ul class="flex gap-4 space-y-2">
                        <li>
                            <a href="https://www.facebook.com/hospital.hdr.leon/">
                                <img src="/icons/facebook.svg" alt="Facebook del Hospital Dr. Raul Hernandez" width="32"
                                    height="32">
                            </a>
                        </li>
                        <li>
                            <a href="https://www.instagram.com/hospitalhrraulhernandez/">
                                <img src="/icons/instagram.svg" alt="Instagram del Hospital Dr. Raul Hernandez"
                                    width="32" height="32">
                            </a>
                        </li>
                    </ul>
                </div>
            </nav>
        </div>
        <hr class="border-t-2 border-gray-400">
        <div class="w-full md:flex bg-secondary-text min-h-20">
            <ul
                class="grid w-full grid-cols-2 grid-rows-2 gap-5 text-center md:justify-between md:items-center md:flex place-items-center">
                <li class="row-start-2">
                    <a href="#" class="hover:underline">Privacidad</a>
                </li>
                <li class="col-span-2">
                    <span>© 2023 - 2026 HOSPITAL DR RAUL HERNANDEZ S.A. de C.V.</span>
                </li>
                <li class="row-start-2">
                    <a href="#" class="hover:underline">Términos y condiciones</a>
                </li>
            </ul>
        </div>
    </footer>
</template>